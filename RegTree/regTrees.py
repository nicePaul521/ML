...
def chooseBestSplit(dataSet,leafType=regLeaf,errType=regErr,ops=(1,4)):
    tolS = ops[0]
    tolN = ops[1]
    #print set(tuple(dataSet[:,-1].T.tolist()[0]))
    if len(set(tuple(dataSet[:,-1].T.tolist()[0]))) == 1:
    ...
    for featIndex in range(n-1):
        for splitVal in set(tuple(dataSet[:,featIndex].T.tolist()[0])):
            #print splitVal
...
def createForeCast(tree,testDat,modelEval=regTreeEval):
    m = len(testDat)
    yHat = mat(zeros((m,1)))
    for i in range(m):
        yHat[i,0] = TreeForeCast(tree,mat(testDat[i]),modelEval)
    return yHat